...
public class CircularQueueByArray {
    ...
    public void deQueue() {
        if (this.isQueueEmpty()) {
            ...
            } else if (this.beginningOfQueue + 1 == this.size) { //if start has reached end of array, then start again from 0
                this.beginningOfQueue = 0;
            } else {
    ...
    public boolean isQueueFull() {
        if (this.topOfQueue + 1 == this.beginningOfQueue) { //If we have completed a circle, then we can say that Queue is full
            return true;
        } else if ((this.beginningOfQueue == 0) && (this.topOfQueue + 1 == this.size)) { //Trivial case of Queue being full
            return true;
        } else {
            return false;
        }
